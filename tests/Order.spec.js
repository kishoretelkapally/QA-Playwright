...
test('@Web test Client App login', async ({ page }) => {
   ...
   const ngStarInsertedElement = page.locator(`table.table > tbody > tr.ng-star-inserted`);
...
for (let i=0;i <optionsCount ; ++i )
   {
      console.log("in for loop for options count");
      const text = await dropdown.locator("button").nth(i).textContent();
      console.log(text);
   if(text.trim() === "India"){
     
      await dropdown.locator("button").nth(i).click();
       console.log("selected india",text);
      break;
   }
}
